/*有理数的堆排序:
题目：自定义有理数类并用最小堆进行排序。
输入：第一行包含一个整数T(1<=T<=100000)；接下来的T行，每一行有两个整数n, d (|n|<103, 0<d<1000)，用空格隔开，表示输入的有理数的分子和分母。
输出：第一行输出有理数的最小堆序列，第二行输出从小到大排序后的序列。其中输出的每个有理数必须规约，以n/d的形式输出，其中d>0且gcd(n,d)=0；如果d=1或n=0则直接输出n。
输入样例：  
5        //有理数个数
3 2      //第1个有理数  3/2
1 3      //第2个有理数  1/3
